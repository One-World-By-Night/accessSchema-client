...
add_action('admin_menu', function () {
    ...
    $slug  = strtolower(ASC_PREFIX) . '_client';
...
function accessSchema_render_admin_page() {
    ...
    wp_nonce_field("{$slug}_clear_roles", "{$slug}_nonce");
    ...
    if (
        isset($_POST["clear_email_{$slug}"], $_POST["{$slug}_nonce"]) &&
        current_user_can('manage_options') &&
        wp_verify_nonce($_POST["{$slug}_nonce"], "{$slug}_clear_roles")
    ) {
